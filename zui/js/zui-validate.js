...
Zui.prototype.validation=dt=>{
    ...
    ipt.each(function(){
        ...
        $(this).blur(function(){
			...
			// 公共校验规则
			// 必选为空
			if(v=='' && ru.required) return verify.error($(this),ru,'不能为空！');
			// 是否为默认值
			for(let i in dft){
				if(dft[i]==v) return verify.error($(this),ru,'不能为默认值！');
			};
			// 长度校验
			if(ru.min!=undefined && v.length<ru.min) return verify.error($(this),ru,'长度不足！');
			if(ru.max!=undefined && v.length>ru.max) return verify.error($(this),ru,'长度超出！');
			if(ru.leng!=undefined && v.length!=ru.leng) return verify.error($(this),ru,'长度不正确！');
			// 调用对应校验方法
			if(vy){
				vy($(this),ru);
			}else{
	...
	const verify={
		...
		name:(n,ru)=>{
			...
			const reg = /^[\u4E00-\u9FA5]+$/;
			// 是否为中文
			if(!reg.test(v)) return verify.error($(this),ru,'必须是中文！');
			// 非法姓名判断
			for(let i=0;i<ne.length;i++){
				if(v.indexOf(ne[i])>=0) return verify.error($(this),ru,'不合法！');
			};
			// 百家姓判断
			if(bjx.indexOf(v.substr(0,1))!=-1||bjx.indexOf(v.substr(0,2))!=-1){
				return verify.zt++;
			}else{
				return verify.error($(this),ru,'姓氏非百家姓！');
			};
		},
		phone:(n,ru)=>{
			const v=n.val();
			const reg = /^(((13[0-9]{1})|(15[0-9]{1})|(17[0-9]{1})|(18[0-9]{1}))+\d{8})$/;
			if(isNaN(v)) return verify.error($(this),ru,'无效！');
			if(!reg.test(v)) return verify.error($(this),ru,'不正确！');
			return verify.zt++;
		},
		number:(n,ru)=>{
			const v=n.val();
			...
			if(isNaN(v)) return verify.error($(this),ru,'无效！');
			// 是否为正数
			if(v<0) return verify.error($(this),ru,'不能为负数！');
			// 小数点位数判断
			if(ru.point && v.indexOf('.')==-1 || v.split('.')[1].length!=ru.point) return verify.error($(this),ru,'请保留位'+ru.point+'小数！');
			return verify.zt++;
		},
		email:(n,ru)=>{
			const v=n.val();
			...
			if(!reg.test(v)){
				return verify.error($(this),ru,'无效！');
			};
			return verify.zt++;
		},
		idcard:(n,ru)=>{
			...
			//取身份证前两位,校验省份 
			if(!idcity[v.substr(0,2)]) return verify.error($(this),ru,'省份不正确！');
			...
			if(om<10) om='0'+om;
			if(od<10) od='0'+od;
			if(oy!=oday.getFullYear() + '/' + om + '/' + od) return verify.error($(this),ru,'日期不正确！');
			//判断年份的范围（1岁到120岁之间)
			const tm = now_year - oday.getFullYear(); 
			if(tm < 1 || tm > 120) return verify.error($(this),ru,'出生年份不正确！');
			...
			if(vn != v.substr(17, 1)) return verify.error($(this),ru,'号码不正确！');
		...
		chinese:(n,ru)=>{
			...
			const reg2 = /[`~!@#$%^&*_+<>，。,.！；;{}\/' ¥%()（）《》？?～·|[]]/im;
			// 是否含符号
			if(reg2.test(v)) return verify.error($(this),ru,'不能包含符号！');
			// 是否为中文
			if(!reg.test(v)) return verify.error($(this),ru,'必须是中文！');
			return verify.zt++;
		},
		error:(n,ru,info)=>{
			zui.prompts(ru.title+info);
		}
	};
